...
class FoamSVGD():
    ...
    def _Kxx_dxKxx(self, X):
        ...
        triu_indices = th.nonzero(squared_dist.triu(1), as_tuple=False).transpose(0, 1)
        ...
        for epoch in range(n_epoch):
            ...
            for batch_idx, (x_data, t_data, k_data, y_data) in enumerate(self.trainingLoader):
                ...
                if ((batch_idx+1) % 500 == 0):
                    self.lg.log('Train Epoch: {} [{}/{} ({:.0f}%)]\t Last sample Loss: {:.6f}\t Last sample Noise: \
                                {:.3f}'.format(epoch+1, (batch_idx+1) * len(x_data), len(self.trainingLoader.dataset),
                                               100. * (batch_idx+1) * len(x_data) / len(self.trainingLoader.dataset),
                                               loss.data.item(), self.models[self.n_samples-1].log_beta.data.item()))

            # Log training loss, MNLL, and mean mse
            ndata = len(self.trainingLoader.dataset)
            self.lg.log("===> Epoch: {}, Scaled loss: {:.6f}, First NN sample log Beta: {:.6f}, Scaled MSE: {:.6f}".
                        format(epoch + 1, training_loss/(ndata*self.n_samples), self.models[0].log_beta.data.item(), training_MSE/ndata))
    ...
    def getTrainingPoints(self, dataManager, n_data = 10000, n_mb = 20, n_valid=0):
        """
    ...
    def getTestingPoints(self, dataManager, n_data=500, n_mb=50):
        """
    ...
    def saveNeuralNet(self, filename):
        ...
        for i in range(self.n_samples):
            th.save(self.models[i].state_dict(), "./torchNets/{}-{}.pth".format(filename,i))
